...
from catalog_stats import compute_statistics, load_papers  # noqa: E402
...
class CatalogStatisticsTest(unittest.TestCase):
    ...
    def test_summary_metrics(self):
        self.assertEqual(self.stats.total, 296)
        self.assertEqual(self.stats.llm_related, 183)
    ...
    def test_distributions_preserve_catalog_total(self):
        ...
        self.assertEqual(self.stats.by_group[0].name, "Assessment & Feedback")
        self.assertEqual(self.stats.by_group[0].count, 86)

    def test_socialcoach_uses_the_specific_social_skill_taxonomy(self):
        socialcoach = next(
            paper for paper in self.papers if paper["title"].startswith("SocialCoach:")
        )
        self.assertEqual(socialcoach["group"], "Specific Scenario")
        self.assertEqual(socialcoach["category"], "Social Skill")
    ...
    def test_generated_artifacts_are_accessible_and_source_labeled(self):
        ...
        svg_outputs = [content for path, content in outputs.items() if path.suffix == ".svg"]
        self.assertTrue(all('role="img"' in content for content in svg_outputs))
        self.assertTrue(all("<title" in content and "<desc" in content for content in svg_outputs))
